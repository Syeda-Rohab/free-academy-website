...
export default function CoursePageClientImpl({ courseInfo }: { courseInfo: Course }) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900/30 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">This course is being updated with new content.</p>
        </div>
      </div>
    );
  }
            ...
            <aside className="lg:w-1/4">
              <div className="glass-card p-6 sticky top-24">
                ...
                <div className="space-y-2">
                  {courseInfo.chapters.map((chapter: any, index: number) => (
                    <motion.button
                      key={chapter.id}
                      whileHover={{ x: 5 }}
                      whileTap={{ scale: 0.98 }}
              ...
              >
                ...
                {currentChapter.introduction && (
                  <div className="bg-gradient-to-r from-blue-500/20 to-cyan-500/20 border border-blue-500/30 rounded-xl p-6 mb-8">
                    ...
                    <p className="text-gray-200 leading-relaxed">{currentChapter.introduction}</p>
                  </div>
                )}
                ...
                {/* Chapter Content */}
                <div className="mb-8">
                  <h3 className="text-2xl font-bold text-white mb-4">Lesson Content</h3>
                  <div className="prose prose-invert max-w-none">
                    <div
                      className="text-gray-200 text-lg leading-relaxed space-y-4"
                      dangerouslySetInnerHTML={{
                        __html: currentChapter.content
                          .replace(/^# (.*$)/gim, '<h1 class="text-2xl font-bold text-blue-400 mt-6 mb-4">$1</h1>')
                          .replace(/^## (.*$)/gim, '<h2 class="text-xl font-bold text-cyan-400 mt-5 mb-3">$1</h2>')
                      ...
                      {currentChapter.codeExamples.map((example: any, index: number) => (
                        ...
                        >
                          ...
                          <pre className="p-4 overflow-x-auto">
                            <code className="text-green-400 text-sm whitespace-pre font-mono">
                              {example.code}
                            </code>
                          </pre>
                          <div className="bg-blue-900/30 px-4 py-3 border-t border-white/10">
                            <p className="text-gray-300 text-sm"><strong className="text-blue-400">Explanation:</strong> {example.explanation}</p>
                          </div>
                        </motion.div>
                      ))}
                    </div>
                ...
                {currentChapter.summary && (
                  <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 rounded-xl p-6 mb-8">
                    ...
                    <p className="text-gray-200 leading-relaxed">{currentChapter.summary}</p>
                  </div>
                )}

                {/* Navigation Buttons */}
                <div className="flex justify-between pt-6 border-t border-white/10 mb-8">
                  <button
                  ...
                  <button
                    onClick={() => {
                      if (selectedChapter < courseInfo.chapters.length - 1) {
                ...
                <div className="border-t border-white/10 pt-8">
                  ...
                  <QuizComponent
                    ...
                    userId="user1"
                    quizQuestions={currentChapter.quiz.questions}
